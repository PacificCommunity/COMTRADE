...
##
   ...
   ##
      ...
      data.frame(Raw_Tuna_Imports_and_Exports[(reporter_desc == "Thailand") & (Year == 2023)])
      # ##
...
## What's the value of the ", nes" partner_desc?
...
# data.frame(Import_Export[str_detect(partner_desc, ", nes"),
                     # list(Total_Net_Wgt   = sum(Total_Net_Wgt,na.rm = TRUE),
                          # Total_Primary_Value = sum(Total_Primary_Value,na.rm = TRUE)),
                      # by = .(Cleaned_Measure, 
                             # Year)])
