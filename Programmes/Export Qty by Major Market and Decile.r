...
##                information. 
##
##                This programme estimates the volume of exported tuna (from the importer's perspective)
##                by major exporting country and price Decile
##
##
##
##
...
##
   ...
   ##
   ## Step 2: Merge data with the Metadata to bring through the conversion factors - we want to remove this and get back to the underlying domestic currency values
   ##
      ...
      Import_Export_in_Domestic_Currency <- Comtrade_Fish_Data[,
                                                               list(Total_Gross_Wgt = sum(gross_wgt,na.rm = TRUE),
                                                                ...
                                                                by = .(Species <- ifelse(cmd_code %in% c('030194', '030195', '030235', '030236', '030345', '030346'), "Bluefin", 
                                                                       ...
                                                                       Year = year(Period),
                                                                       ReporterLocation <- ifelse(reporter_desc %in% c("American Samoa","Areas, nes","Australia","Cambodia","China","China, Hong Kong SAR","China, Macao SAR","Cook Isds",
                                                                       ...
                                                                       PartnerLocation <- ifelse(partner_desc %in% c("American Samoa","Areas, nes","Australia","Cambodia","China","China, Hong Kong SAR","China, Macao SAR","Cook Isds",
                                                                                                                       "Fiji","France","French Polynesia","FS Micronesia","Guam","Indonesia","Kiribati","Malaysia","Marshall Isds",
                                                                                                                       "N. Mariana Isds","Nauru","New Caledonia", "New Zealand","Oceania, nes","Other Asia, nes","Palau","Papua New Guinea","Philippines",
                                                                                                                       "Pitcairn","Rep. of Korea","Samoa","Solomon Isds","Thailand","Tokelau","Tonga","Tuvalu","Vanuatu","Viet Nam",
                                                                                                                       "Christmas Isds","Japan","Niue","Singapore","Timor-Leste","Wallis and Futuna Isds","USA"), "Pacific", "Non-Pacific"), 
   ...
   ##
      ...
      Peek <- data.frame(Import_Export_in_Domestic_and_USD_Currency[Total_Net_Wgt > 0,
                                                         list(Total_Net_Wgt        = sum(Total_Net_Wgt,na.rm = TRUE),
                                                              ...
                                                              primary_value        = sum(primary_value,na.rm = TRUE),
                                                              Implicit_Price_CurrentUSD = sum(primary_value,na.rm = TRUE) / sum(Total_Net_Wgt,na.rm = TRUE),
                                                              Implicit_Price_ConstantUSD = sum(PrimaryValue_USD2019,na.rm = TRUE) / sum(Total_Net_Wgt,na.rm = TRUE)),
                                                          by = .(Year,
                                                                 Species,
                                                                 ReporterLocation,
                                                                 reporter_desc,
      ...
      ##
      ##    Allocate the countries to price deciles for each year.
      ##       Unlike Fresh_Tuna_Analysis_Version2, I'm going to create the deciles on constant price measures, so what's 
      ##       distinguishing the different prices is tuna-related factors only, not potentially country specific inflation. 
      ##
      ...
      for(S in 1:length(Species))
      {
         for(j in 1:length(Cleaned_Measure))
         {
            for(i in 1:length(Years))
            {
              ...
              if(nrow(One_Year) > 0)
                {
                 Deciles <- data.frame(Value = quantile(One_Year$Implicit_Price_ConstantUSD, prob = seq(0, 1, length = 11), type = 5, na.rm = TRUE))
                 ...
                 Deciles$Year <- Years[i]
                  ...
                  for(i in 1:nrow(One_Year))
                     {
                        One_Year$Decile[i]      <- Deciles$DGroup[((One_Year$Implicit_Price_ConstantUSD[i] >= Deciles$Value.x ) &
                                                                   (One_Year$Implicit_Price_ConstantUSD[i] <= Deciles$Value.y ))]
                        One_Year$DecileGroup[i] <- Deciles$ID[((One_Year$Implicit_Price_ConstantUSD[i] >= Deciles$Value.x ) &
                                                               (One_Year$Implicit_Price_ConstantUSD[i] <= Deciles$Value.y ))]
                     }  
...
##               
   ...
   ##               
      Export_Volumes <- aggregate(Total_Net_Wgt ~ Species + Decile + Year + PartnerLocation + Major,
                              data = Results,
                              FUN  = sum,
                              subset = Year < 2024,
                              na.action = NULL)

      AverVolumes <- aggregate(Total_Net_Wgt ~ Species + Decile + PartnerLocation + Major,
                              data = Export_Volumes,
                              FUN  = mean,
                              subset = Year < 2022,
                              na.action = NULL)
                              
      names(AverVolumes)[5] ="Average_over_Time"

      Export_Volumes <- merge(Export_Volumes,
                              AverVolumes,
                              by = c("Species", "Decile", "PartnerLocation", "Major"))
...
for(j in 1:length(Species))
{
   ...
   for(i in 1:length(Deciles))
   {
      ...
      ggplot(Export_Volumes[(Export_Volumes$Species == Species[j]) &
       ...
       geom_line(aes(x=Year, y=Total_Net_Wgt/1000), colour = SPCColours("Green"), size = 0.7) +
       geom_line(aes(x=Year, y=Average_over_Time/1000), linetype = "dashed", colour = SPCColours("Light_Blue"), size = 0.7) +
       facet_grid( . ~ PartnerLocation + Major_Short, scales = "free", space = "free") +
       ...
       ggsave(paste0("Graphical_Output/",Species[j], " Exported Tuna Volumes - Major Markets - Price Decile ", i,".png"), height =2*16.13, width = 2*20.66, dpi = 165, units = c("cm"))
   }
}      
##
##    And we're done
