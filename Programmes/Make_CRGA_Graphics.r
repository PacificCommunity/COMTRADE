...
##
   ...
   ##
      ...
      ##   Aggregate metrics
      ##
         Volume_Aggregates <- Catch_Volume[(Spreadsheet == "WCPFC-CA_tuna_fisheries_2024") & (Measure == "Skipjack"),
                                           list(Total_Metric_Tonnes = sum(value,na.rm = TRUE)),
                                           by = .(Measure, 
                                                  Year = as.numeric(Year))]
         ggplot(Volume_Aggregates[Measure != ""], 
                   aes(x = Year, 
                   ...
                   labs(title = "Skipjack - Total Volume Caught\n", 
...
FFA_Compendium_of_Economic_and_Development_Statistics_2024
